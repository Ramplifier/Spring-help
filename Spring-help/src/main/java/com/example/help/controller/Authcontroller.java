...
import com.example.help.Payloads.LoginRequest;
import com.example.help.Payloads.SignupRequest;
import com.example.help.container.UserContainer;
...
import com.example.help.repository.UserRepository;
...
public class Authcontroller {
    ...
    private Set<Role> bepaalRollen(Set<RoleEnum> rollen) {
        Set<Role> lijst = new HashSet<>();
        //Als er geen rollen zijn in het request geef dan allen user rechten terug
        if(rollen == null){

            lijst.add(roleRepository.findByName(RoleEnum.USER));
            return lijst;
        }else{
            for (RoleEnum r: rollen) {
